...
<script>
    ...
    // });
    // 1. Use $(document).on to prevent "null" crashes
    $(document).on('show.bs.modal', '#viewIncidentModal', function(event) {
        // 'event.relatedTarget' is the button that was clicked
        const button = $(event.relatedTarget);

        // 2. Extract info from the button's data attributes
        ...
        // Update your global ID variable if you need it elsewhere
        currentIncidentId = incidentId;

        console.log("Incident Modal Loaded for ID:", incidentId);
    });
    ...
    $(document).on('click', '.deleteIncidentBtn', function() {
    ...
    $(document).on('click', '.editAlertsBtn', function() {
        ...
        row.find('.alert-message').html(`
        <input type="text"
               class="form-control edit-alert-message"
               value="${message}">
    `);

        row.find('.alert-severity').html(`
        <select class="form-select edit-alert-severity">
    ...
    `);
        ...
        row.find('.alert-region').html(`
        <input type="text"
               class="form-control edit-alert-region"
               value="${region}">
    `);

        row.find('.alert-status').html(`
        <select class="form-select edit-alert-status">
    ...
    `);
        ...
        // Replace buttons
        row.find('td:last').html(`
        <button class="btn btn-success btn-sm saveAlertBtn" data-id="${$(this).data('id')}">
            Save
        </button>

        <button class="btn btn-secondary btn-sm cancelAlertBtn" data-id="${$(this).data('id')}">
            Cancel
        </button>
    `);
    ...
    $(document).ready(function() {
        ...
        $('.category-filter').on('click', function() {
            ...
            $('.category-filter').css('border-color', 'transparent');
            $(this).css('border-color', '#4318ff');
    ...
    $(document).ready(function() {
        ...
        var table = $('#newsTable').DataTable();
        console.log("2. Table initialized:", table);

        $('.category-filter').on('click', function() {
            console.log("3. Category clicked!");
